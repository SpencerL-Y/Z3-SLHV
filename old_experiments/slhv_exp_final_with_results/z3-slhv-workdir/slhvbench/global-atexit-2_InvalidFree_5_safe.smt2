(set-logic SLHV)
(declare-hvar emp IntHeap)
(declare-locvar nil IntLoc)
(declare-datatype pt_record_0 ((Pt_R_0 (loc IntLoc))))
(declare-datatype pt_record_1 ((Pt_R_1 (data Int))))
(declare-const invalidFree_5 Bool)
(declare-const loc_5 Int)
(declare-const invalidFree_4 Bool)
(declare-hvar H_4 IntHeap)
(declare-hvar H_5 IntHeap)
(declare-hvar AH_4 IntHeap)
(declare-hvar AH_5 IntHeap)
(declare-const $r_main0_4 Int)
(declare-const $r_main0_5 Int)
(declare-const $i9_main0_4 Int)
(declare-const $i9_main0_5 Int)
(declare-const $i5_main0_4 Int)
(declare-const $i5_main0_5 Int)
(declare-locvar $p8_main0_4 IntLoc)
(declare-locvar $p8_main0_5 IntLoc)
(declare-locvar $p7_main0_4 IntLoc)
(declare-locvar $p7_main0_5 IntLoc)
(declare-locvar $p6_main0_4 IntLoc)
(declare-locvar $p6_main0_5 IntLoc)
(declare-locvar $p4_main0_4 IntLoc)
(declare-locvar $p4_main0_5 IntLoc)
(declare-locvar $p3_main0_4 IntLoc)
(declare-locvar $p3_main0_5 IntLoc)
(declare-locvar $p2_main0_4 IntLoc)
(declare-locvar $p2_main0_5 IntLoc)
(declare-locvar $p1_main0_4 IntLoc)
(declare-locvar $p1_main0_5 IntLoc)
(declare-locvar $p17_main0_4 IntLoc)
(declare-locvar $p17_main0_5 IntLoc)
(declare-locvar $p16_main0_4 IntLoc)
(declare-locvar $p16_main0_5 IntLoc)
(declare-locvar $p15_main0_4 IntLoc)
(declare-locvar $p15_main0_5 IntLoc)
(declare-locvar $p14_main0_4 IntLoc)
(declare-locvar $p14_main0_5 IntLoc)
(declare-locvar $p13_main0_4 IntLoc)
(declare-locvar $p13_main0_5 IntLoc)
(declare-locvar $p12_main0_4 IntLoc)
(declare-locvar $p12_main0_5 IntLoc)
(declare-locvar $p11_main0_4 IntLoc)
(declare-locvar $p11_main0_5 IntLoc)
(declare-locvar $p10_main0_4 IntLoc)
(declare-locvar $p10_main0_5 IntLoc)
(declare-locvar $p0_main0_4 IntLoc)
(declare-locvar $p0_main0_5 IntLoc)
(declare-locvar $M.0_4 IntLoc)
(declare-locvar $M.0_5 IntLoc)
(declare-const loc_4 Int)
(declare-const invalidFree_3 Bool)
(declare-hvar H_3 IntHeap)
(declare-hvar AH_3 IntHeap)
(declare-const $r_main0_3 Int)
(declare-const $i9_main0_3 Int)
(declare-const $i5_main0_3 Int)
(declare-locvar $p8_main0_3 IntLoc)
(declare-locvar $p7_main0_3 IntLoc)
(declare-locvar $p6_main0_3 IntLoc)
(declare-locvar $p4_main0_3 IntLoc)
(declare-locvar $p3_main0_3 IntLoc)
(declare-locvar $p2_main0_3 IntLoc)
(declare-locvar $p1_main0_3 IntLoc)
(declare-locvar $p17_main0_3 IntLoc)
(declare-locvar $p16_main0_3 IntLoc)
(declare-locvar $p15_main0_3 IntLoc)
(declare-locvar $p14_main0_3 IntLoc)
(declare-locvar $p13_main0_3 IntLoc)
(declare-locvar $p12_main0_3 IntLoc)
(declare-locvar $p11_main0_3 IntLoc)
(declare-locvar $p10_main0_3 IntLoc)
(declare-locvar $p0_main0_3 IntLoc)
(declare-locvar $M.0_3 IntLoc)
(declare-const loc_3 Int)
(declare-const $r_main01_4 Int)
(declare-const invalidFree_2 Bool)
(declare-hvar H_2 IntHeap)
(declare-hvar AH_2 IntHeap)
(declare-const $i9_main0_2 Int)
(declare-const $i5_main0_2 Int)
(declare-locvar $p8_main0_2 IntLoc)
(declare-locvar $p7_main0_2 IntLoc)
(declare-locvar $p6_main0_2 IntLoc)
(declare-locvar $p4_main0_2 IntLoc)
(declare-locvar $p3_main0_2 IntLoc)
(declare-locvar $p2_main0_2 IntLoc)
(declare-locvar $p1_main0_2 IntLoc)
(declare-locvar $p17_main0_2 IntLoc)
(declare-locvar $p16_main0_2 IntLoc)
(declare-locvar $p15_main0_2 IntLoc)
(declare-locvar $p14_main0_2 IntLoc)
(declare-locvar $p13_main0_2 IntLoc)
(declare-locvar $p12_main0_2 IntLoc)
(declare-locvar $p11_main0_2 IntLoc)
(declare-locvar $p10_main0_2 IntLoc)
(declare-locvar $p0_main0_2 IntLoc)
(declare-locvar $M.0_2 IntLoc)
(declare-const $r_main01_3 Int)
(declare-const loc_2 Int)
(declare-const $r_main0_2 Int)
(declare-const invalidFree4_3 Bool)
(declare-hvar H7_3 IntHeap)
(declare-hvar AH6_3 IntHeap)
(declare-locvar $p12_main01_3 IntLoc)
(declare-locvar $p11_main01_3 IntLoc)
(declare-locvar $p10_main01_3 IntLoc)
(declare-hvar h10_3 IntHeap)
(declare-locvar l16_3 IntLoc)
(declare-const d17_3 Int)
(declare-const d15_3 Int)
(declare-locvar l15_3 IntLoc)
(declare-const d16_3 Int)
(declare-hvar ah7 IntHeap)
(declare-const d18_3 Int)
(declare-hvar ah8_3 IntHeap)
(declare-locvar l13_3 IntLoc)
(declare-hvar h8_3 IntHeap)
(declare-const $r_main0_1 Int)
(declare-const $i9_main0_1 Int)
(declare-const $i5_main0_1 Int)
(declare-locvar $p8_main0_1 IntLoc)
(declare-locvar $p7_main0_1 IntLoc)
(declare-locvar $p6_main0_1 IntLoc)
(declare-locvar $p4_main0_1 IntLoc)
(declare-locvar $p3_main0_1 IntLoc)
(declare-locvar $p2_main0_1 IntLoc)
(declare-locvar $p1_main0_1 IntLoc)
(declare-locvar $p12_main0_1 IntLoc)
(declare-locvar $p11_main0_1 IntLoc)
(declare-locvar $p10_main0_1 IntLoc)
(declare-locvar $p0_main0_1 IntLoc)
(declare-const invalidFree3_2 Bool)
(declare-hvar H6_2 IntHeap)
(declare-hvar AH5_2 IntHeap)
(declare-locvar $p17_main01_2 IntLoc)
(declare-locvar $p16_main01_2 IntLoc)
(declare-locvar $p15_main01_2 IntLoc)
(declare-locvar $p14_main01_2 IntLoc)
(declare-locvar $p13_main01_2 IntLoc)
(declare-locvar $M.04_2 IntLoc)
(declare-const invalidFree2_2 Bool)
(declare-hvar h7_2 IntHeap)
(declare-locvar l12_2 IntLoc)
(declare-const d13_2 Int)
(declare-hvar H5_2 IntHeap)
(declare-const d11_2 Int)
(declare-locvar l11_2 IntLoc)
(declare-const d12_2 Int)
(declare-hvar ah5 IntHeap)
(declare-hvar AH4_2 IntHeap)
(declare-const d14_2 Int)
(declare-hvar ah6_2 IntHeap)
(declare-locvar $M.0_1 IntLoc)
(declare-const invalidFree_1 Bool)
(declare-hvar h6_2 IntHeap)
(declare-locvar l10_2 IntLoc)
(declare-const d9_2 Int)
(declare-hvar H_1 IntHeap)
(declare-const d7_2 Int)
(declare-locvar l9_2 IntLoc)
(declare-const d8_2 Int)
(declare-hvar ah3 IntHeap)
(declare-hvar AH_1 IntHeap)
(declare-const d10_2 Int)
(declare-hvar ah4_2 IntHeap)
(declare-locvar l7_2 IntLoc)
(declare-hvar h4_2 IntHeap)
(declare-const loc_1 Int)
(declare-locvar $p17_main0_1 IntLoc)
(declare-locvar $p16_main0_1 IntLoc)
(declare-locvar $p15_main0_1 IntLoc)
(declare-locvar $p14_main0_1 IntLoc)
(declare-locvar $p13_main0_1 IntLoc)
(declare-const invalidFree1_2 Bool)
(declare-hvar H4_2 IntHeap)
(declare-hvar AH3_2 IntHeap)
(declare-locvar $p8_main01_2 IntLoc)
(declare-locvar $p7_main01_2 IntLoc)
(declare-locvar $p6_main01_2 IntLoc)
(declare-const $i9_main01_2 Int)
(declare-locvar $M.03_2 IntLoc)
(declare-hvar h3_2 IntHeap)
(declare-locvar l6_2 IntLoc)
(declare-const d5_2 Int)
(declare-const d3_2 Int)
(declare-locvar l5_2 IntLoc)
(declare-const d4_2 Int)
(declare-hvar ah1 IntHeap)
(declare-const d6_2 Int)
(declare-hvar ah2_2 IntHeap)
(declare-const invalidFree_0 Bool)
(declare-const $r_main0_0 Int)
(declare-const $i9_main0_0 Int)
(declare-const $i5_main0_0 Int)
(declare-locvar $p8_main0_0 IntLoc)
(declare-locvar $p7_main0_0 IntLoc)
(declare-locvar $p6_main0_0 IntLoc)
(declare-locvar $p17_main0_0 IntLoc)
(declare-locvar $p16_main0_0 IntLoc)
(declare-locvar $p15_main0_0 IntLoc)
(declare-locvar $p14_main0_0 IntLoc)
(declare-locvar $p13_main0_0 IntLoc)
(declare-locvar $p12_main0_0 IntLoc)
(declare-locvar $p11_main0_0 IntLoc)
(declare-locvar $p10_main0_0 IntLoc)
(declare-hvar H3_1 IntHeap)
(declare-hvar AH2_1 IntHeap)
(declare-locvar $p4_main01_1 IntLoc)
(declare-locvar $p3_main01_1 IntLoc)
(declare-locvar $p2_main01_1 IntLoc)
(declare-locvar $p1_main01_1 IntLoc)
(declare-locvar $p0_main01_1 IntLoc)
(declare-locvar $M.02_1 IntLoc)
(declare-hvar h1_1 IntHeap)
(declare-locvar l3_1 IntLoc)
(declare-hvar H2_1 IntHeap)
(declare-locvar l2_1 IntLoc)
(declare-hvar AH1_1 IntHeap)
(declare-const d2_1 Int)
(declare-hvar H1_1 IntHeap)
(declare-locvar l1_1 IntLoc)
(declare-hvar AH_0 IntHeap)
(declare-const d1_1 Int)
(declare-hvar H_0 IntHeap)
(declare-locvar $M.01_1 IntLoc)
(declare-const loc_0 Int)
(assert
 (let (($x1334 (= loc_5 5)))
 (let (($x1527 (= invalidFree_5 invalidFree_4)))
 (let (($x1327 (= H_5 H_4)))
 (let (($x1328 (and (and true (= AH_5 AH_4)) $x1327)))
 (let (($x1320 (= $r_main0_5 $r_main0_4)))
 (let (($x1317 (= $i9_main0_5 $i9_main0_4)))
 (let (($x1321 (and (and (and true (= $i5_main0_5 $i5_main0_4)) $x1317) $x1320)))
 (let (($x1310 (= $p8_main0_5 $p8_main0_4)))
 (let (($x1307 (= $p7_main0_5 $p7_main0_4)))
 (let (($x1304 (= $p6_main0_5 $p6_main0_4)))
 (let (($x1301 (= $p4_main0_5 $p4_main0_4)))
 (let (($x1298 (= $p3_main0_5 $p3_main0_4)))
 (let (($x1295 (= $p2_main0_5 $p2_main0_4)))
 (let (($x1292 (= $p1_main0_5 $p1_main0_4)))
 (let (($x1289 (= $p17_main0_5 $p17_main0_4)))
 (let (($x1286 (= $p16_main0_5 $p16_main0_4)))
 (let (($x1283 (= $p15_main0_5 $p15_main0_4)))
 (let (($x1280 (= $p14_main0_5 $p14_main0_4)))
 (let (($x1277 (= $p13_main0_5 $p13_main0_4)))
 (let (($x1274 (= $p12_main0_5 $p12_main0_4)))
 (let (($x1271 (= $p11_main0_5 $p11_main0_4)))
 (let (($x1268 (= $p10_main0_5 $p10_main0_4)))
 (let (($x1265 (= $p0_main0_5 $p0_main0_4)))
 (let (($x1275 (and (and (and (and (and true (= $M.0_5 $M.0_4)) $x1265) $x1268) $x1271) $x1274)))
 (let (($x1293 (and (and (and (and (and (and $x1275 $x1277) $x1280) $x1283) $x1286) $x1289) $x1292)))
 (let (($x1311 (and (and (and (and (and (and $x1293 $x1295) $x1298) $x1301) $x1304) $x1307) $x1310)))
 (let (($x1524 (and (and (and (or true invalidFree_4) (and true true)) $x1311) $x1321)))
 (let (($x1244 (= loc_4 5)))
 (let (($x1507 (= invalidFree_4 invalidFree_3)))
 (let (($x1237 (= H_4 H_3)))
 (let (($x1238 (and (and true (= AH_4 AH_3)) $x1237)))
 (let (($x1250 (= $r_main0_4 $r_main0_3)))
 (let (($x1230 (= $i9_main0_4 $i9_main0_3)))
 (let (($x1231 (and (and true (= $i5_main0_4 $i5_main0_3)) $x1230)))
 (let (($x1251 (and $x1231 $x1250)))
 (let (($x1223 (= $p8_main0_4 $p8_main0_3)))
 (let (($x1220 (= $p7_main0_4 $p7_main0_3)))
 (let (($x1217 (= $p6_main0_4 $p6_main0_3)))
 (let (($x1214 (= $p4_main0_4 $p4_main0_3)))
 (let (($x1211 (= $p3_main0_4 $p3_main0_3)))
 (let (($x1208 (= $p2_main0_4 $p2_main0_3)))
 (let (($x1205 (= $p1_main0_4 $p1_main0_3)))
 (let (($x1202 (= $p17_main0_4 $p17_main0_3)))
 (let (($x1199 (= $p16_main0_4 $p16_main0_3)))
 (let (($x1196 (= $p15_main0_4 $p15_main0_3)))
 (let (($x1193 (= $p14_main0_4 $p14_main0_3)))
 (let (($x1190 (= $p13_main0_4 $p13_main0_3)))
 (let (($x1187 (= $p12_main0_4 $p12_main0_3)))
 (let (($x1184 (= $p11_main0_4 $p11_main0_3)))
 (let (($x1181 (= $p10_main0_4 $p10_main0_3)))
 (let (($x1178 (= $p0_main0_4 $p0_main0_3)))
 (let (($x1188 (and (and (and (and (and true (= $M.0_4 $M.0_3)) $x1178) $x1181) $x1184) $x1187)))
 (let (($x1206 (and (and (and (and (and (and $x1188 $x1190) $x1193) $x1196) $x1199) $x1202) $x1205)))
 (let (($x1224 (and (and (and (and (and (and $x1206 $x1208) $x1211) $x1214) $x1217) $x1220) $x1223)))
 (let (($x1514 (and (and (and (or true invalidFree_3) (and true true)) $x1224) $x1251)))
 (let (($x1162 (= loc_3 5)))
 (let (($x1171 (= $r_main0_4 $r_main01_4)))
 (let (($x1172 (and $x1171 true)))
 (let (($x1168 (= $r_main01_4 0)))
 (let (($x1501 (or $x1168 invalidFree_3)))
 (let (($x1509 (and (and (and (and (and (and $x1501 $x1172) $x1224) $x1231) $x1238) $x1507) $x1244)))
 (let (($x1113 (= loc_3 4)))
 (let (($x1519 (and (=> $x1113 $x1509) (=> $x1162 (and (and (and $x1514 $x1238) $x1507) $x1244)))))
 (let (($x1486 (= invalidFree_3 invalidFree_2)))
 (let (($x1143 (= H_3 H_2)))
 (let (($x1144 (and (and true (= AH_3 AH_2)) $x1143)))
 (let (($x1106 (= $i9_main0_3 $i9_main0_2)))
 (let (($x1107 (and (and true (= $i5_main0_3 $i5_main0_2)) $x1106)))
 (let (($x1099 (= $p8_main0_3 $p8_main0_2)))
 (let (($x1096 (= $p7_main0_3 $p7_main0_2)))
 (let (($x1093 (= $p6_main0_3 $p6_main0_2)))
 (let (($x1090 (= $p4_main0_3 $p4_main0_2)))
 (let (($x1087 (= $p3_main0_3 $p3_main0_2)))
 (let (($x1084 (= $p2_main0_3 $p2_main0_2)))
 (let (($x1081 (= $p1_main0_3 $p1_main0_2)))
 (let (($x1078 (= $p17_main0_3 $p17_main0_2)))
 (let (($x1075 (= $p16_main0_3 $p16_main0_2)))
 (let (($x1072 (= $p15_main0_3 $p15_main0_2)))
 (let (($x1069 (= $p14_main0_3 $p14_main0_2)))
 (let (($x1066 (= $p13_main0_3 $p13_main0_2)))
 (let (($x1125 (= $p12_main0_3 $p12_main0_2)))
 (let (($x1123 (= $p11_main0_3 $p11_main0_2)))
 (let (($x1121 (= $p10_main0_3 $p10_main0_2)))
 (let (($x1063 (= $p0_main0_3 $p0_main0_2)))
 (let (($x1064 (and (and true (= $M.0_3 $M.0_2)) $x1063)))
 (let (($x1129 (and (and (and (and (and (and $x1064 $x1121) $x1123) $x1125) $x1066) $x1069) $x1072)))
 (let (($x1135 (and (and (and (and (and (and $x1129 $x1075) $x1078) $x1081) $x1084) $x1087) $x1090)))
 (let (($x1138 (and (and (and $x1135 $x1093) $x1096) $x1099)))
 (let (($x1154 (= $r_main0_3 $r_main01_3)))
 (let (($x1155 (and $x1154 true)))
 (let (($x1152 (= $r_main01_3 0)))
 (let (($x1491 (or $x1152 invalidFree_2)))
 (let (($x1497 (and (and (and (and (and (and $x1491 $x1155) $x1138) $x1107) $x1144) $x1486) $x1162)))
 (let (($x965 (= loc_2 4)))
 (let (($x1109 (= $r_main0_3 $r_main0_2)))
 (let (($x1110 (and $x1107 $x1109)))
 (let (($x1484 (and (and (and (or true invalidFree_2) (and true true)) $x1138) $x1110)))
 (let (($x808 (= loc_2 3)))
 (let (($x1117 (and $x808 (not (= $i9_main0_2 1)))))
 (let (($x1082 (and (and (and (and (and (and $x1064 $x1066) $x1069) $x1072) $x1075) $x1078) $x1081)))
 (let (($x1100 (and (and (and (and (and (and $x1082 $x1084) $x1087) $x1090) $x1093) $x1096) $x1099)))
 (let (($x1474 (= invalidFree_3 invalidFree4_3)))
 (let (($x1053 (= H_3 H7_3)))
 (let (($x1050 (= AH_3 AH6_3)))
 (let (($x1047 (= $p12_main0_3 $p12_main01_3)))
 (let (($x1044 (= $p11_main0_3 $p11_main01_3)))
 (let (($x1042 (= $p10_main0_3 $p10_main01_3)))
 (let (($x1054 (and (and (and (and $x1042 $x1044) $x1047) $x1050) $x1053)))
 (let (($x1455 (= invalidFree4_3 invalidFree_2)))
 (let (($x1456 (and invalidFree_2 $x1455)))
 (let (($x988 (= H7_3 h10_3)))
 (let (($x989 (= $p12_main01_3 l16_3)))
 (let (($x993 (= H_2 (uplus h10_3 (pt l16_3 (Pt_R_1 d17_3))))))
 (let (($x996 (= d15_3 1)))
 (let (($x998 (= $p12_main01_3 l15_3)))
 (let (($x1002 (= H_2 (uplus h10_3 (pt l15_3 (Pt_R_1 d16_3))))))
 (let (($x1005 (= d15_3 2)))
 (let (($x1007 (and (=> $x1005 (and (and $x1002 $x998) $x988)) (=> $x996 (and (and $x993 $x989) $x988)))))
 (let (($x1008 (= AH6_3 ah7)))
 (let (($x1012 (= AH_2 (uplus ah7 (pt $p12_main01_3 (Pt_R_1 d15_3))))))
 (let (($x1014 (and (and $x1012 $x1008) $x1007)))
 (let (($x1463 (or (= $p12_main01_3 nil) (= ah8_3 (uplus AH_2 (pt $p12_main01_3 (Pt_R_1 d18_3)))))))
 (let (($x1464 (and $x1463 invalidFree4_3)))
 (let (($x1016 (= $p12_main01_3 $p11_main01_3)))
 (let (($x1467 (or $x1016 invalidFree_2)))
 (let (($x1025 (and (= H_2 (uplus h8_3 (pt $p10_main01_3 (Pt_R_0 l13_3)))) (= $p11_main01_3 l13_3))))
 (let (($x1468 (or $x1025 invalidFree_2)))
 (let (($x1036 (= $p10_main01_3 $M.0_2)))
 (let (($x1469 (or $x1036 invalidFree_2)))
 (let (($x1472 (and (and (and $x1469 $x1468) $x1467) (or (or $x1464 (and $x1014 $x1455)) $x1456))))
 (let (($x986 (= $i9_main0_2 1)))
 (let (($x987 (and $x808 $x986)))
 (let (($x1480 (=> $x987 (and (and (and (and $x1472 (and $x1054 $x1474)) $x1100) $x1110) $x1113))))
 (let (($x1499 (and (and $x1480 (=> $x1117 (and (and (and $x1484 $x1144) $x1486) $x1113))) (=> $x965 $x1497))))
 (let (($x800 (= $r_main0_2 $r_main0_1)))
 (let (($x960 (= $i9_main0_2 $i9_main0_1)))
 (let (($x797 (= $i5_main0_2 $i5_main0_1)))
 (let (($x798 (and true $x797)))
 (let (($x962 (and (and $x798 $x960) $x800)))
 (let (($x957 (= $p8_main0_2 $p8_main0_1)))
 (let (($x955 (= $p7_main0_2 $p7_main0_1)))
 (let (($x953 (= $p6_main0_2 $p6_main0_1)))
 (let (($x793 (= $p4_main0_2 $p4_main0_1)))
 (let (($x790 (= $p3_main0_2 $p3_main0_1)))
 (let (($x787 (= $p2_main0_2 $p2_main0_1)))
 (let (($x784 (= $p1_main0_2 $p1_main0_1)))
 (let (($x766 (= $p12_main0_2 $p12_main0_1)))
 (let (($x763 (= $p11_main0_2 $p11_main0_1)))
 (let (($x760 (= $p10_main0_2 $p10_main0_1)))
 (let (($x767 (and (and (and (and true (= $p0_main0_2 $p0_main0_1)) $x760) $x763) $x766)))
 (let (($x956 (and (and (and (and (and (and $x767 $x784) $x787) $x790) $x793) $x953) $x955)))
 (let (($x958 (and $x956 $x957)))
 (let (($x1443 (= invalidFree_2 invalidFree3_2)))
 (let (($x944 (= H_2 H6_2)))
 (let (($x942 (= AH_2 AH5_2)))
 (let (($x940 (= $p17_main0_2 $p17_main01_2)))
 (let (($x938 (= $p16_main0_2 $p16_main01_2)))
 (let (($x936 (= $p15_main0_2 $p15_main01_2)))
 (let (($x934 (= $p14_main0_2 $p14_main01_2)))
 (let (($x935 (and (and (= $M.0_2 $M.04_2) (= $p13_main0_2 $p13_main01_2)) $x934)))
 (let (($x945 (and (and (and (and (and $x935 $x936) $x938) $x940) $x942) $x944)))
 (let (($x840 (= $M.04_2 nil)))
 (let (($x1406 (or $x840 invalidFree3_2)))
 (let (($x1407 (= invalidFree3_2 invalidFree2_2)))
 (let (($x1408 (and invalidFree2_2 $x1407)))
 (let (($x842 (= H6_2 h7_2)))
 (let (($x843 (= $p17_main01_2 l12_2)))
 (let (($x847 (= H5_2 (uplus h7_2 (pt l12_2 (Pt_R_1 d13_2))))))
 (let (($x850 (= d11_2 1)))
 (let (($x852 (= $p17_main01_2 l11_2)))
 (let (($x856 (= H5_2 (uplus h7_2 (pt l11_2 (Pt_R_1 d12_2))))))
 (let (($x859 (= d11_2 2)))
 (let (($x861 (and (=> $x859 (and (and $x856 $x852) $x842)) (=> $x850 (and (and $x847 $x843) $x842)))))
 (let (($x862 (= AH5_2 ah5)))
 (let (($x866 (= AH4_2 (uplus ah5 (pt $p17_main01_2 (Pt_R_1 d11_2))))))
 (let (($x868 (and (and $x866 $x862) $x861)))
 (let (($x1415 (or (= $p17_main01_2 nil) (= ah6_2 (uplus AH4_2 (pt $p17_main01_2 (Pt_R_1 d14_2)))))))
 (let (($x1416 (and $x1415 invalidFree3_2)))
 (let (($x870 (= $p17_main01_2 $p16_main01_2)))
 (let (($x1419 (or $x870 invalidFree2_2)))
 (let (($x872 (= $p16_main01_2 $M.0_1)))
 (let (($x1420 (or $x872 invalidFree2_2)))
 (let (($x1421 (= invalidFree2_2 invalidFree_1)))
 (let (($x1422 (and invalidFree_1 $x1421)))
 (let (($x874 (= H5_2 h6_2)))
 (let (($x875 (= $p15_main01_2 l10_2)))
 (let (($x879 (= H_1 (uplus h6_2 (pt l10_2 (Pt_R_1 d9_2))))))
 (let (($x882 (= d7_2 1)))
 (let (($x884 (= $p15_main01_2 l9_2)))
 (let (($x888 (= H_1 (uplus h6_2 (pt l9_2 (Pt_R_1 d8_2))))))
 (let (($x891 (= d7_2 2)))
 (let (($x893 (and (=> $x891 (and (and $x888 $x884) $x874)) (=> $x882 (and (and $x879 $x875) $x874)))))
 (let (($x894 (= AH4_2 ah3)))
 (let (($x898 (= AH_1 (uplus ah3 (pt $p15_main01_2 (Pt_R_1 d7_2))))))
 (let (($x900 (and (and $x898 $x894) $x893)))
 (let (($x1429 (or (= $p15_main01_2 nil) (= ah4_2 (uplus AH_1 (pt $p15_main01_2 (Pt_R_1 d10_2)))))))
 (let (($x1430 (and $x1429 invalidFree2_2)))
 (let (($x902 (= $p15_main01_2 $p14_main01_2)))
 (let (($x1433 (or $x902 invalidFree_1)))
 (let (($x911 (and (= H_1 (uplus h4_2 (pt $p13_main01_2 (Pt_R_0 l7_2)))) (= $p14_main01_2 l7_2))))
 (let (($x1434 (or $x911 invalidFree_1)))
 (let (($x922 (= $p13_main01_2 $M.0_1)))
 (let (($x1435 (or $x922 invalidFree_1)))
 (let (($x1438 (and (and (and $x1435 $x1434) $x1433) (or (or $x1430 (and $x900 $x1421)) $x1422))))
 (let (($x1441 (and (and (and $x1438 $x1420) $x1419) (or (or $x1416 (and $x868 $x1407)) $x1408))))
 (let (($x1448 (and (and (and (and (and $x1441 $x1406) (and $x945 $x1443)) $x958) $x962) $x965)))
 (let (($x666 (= loc_1 2)))
 (let (($x839 (and $x666 (not (= $i5_main0_1 1)))))
 (let (($x801 (and $x798 $x800)))
 (let (($x781 (= $p17_main0_2 $p17_main0_1)))
 (let (($x778 (= $p16_main0_2 $p16_main0_1)))
 (let (($x775 (= $p15_main0_2 $p15_main0_1)))
 (let (($x772 (= $p14_main0_2 $p14_main0_1)))
 (let (($x779 (and (and (and (and $x767 (= $p13_main0_2 $p13_main0_1)) $x772) $x775) $x778)))
 (let (($x794 (and (and (and (and (and $x779 $x781) $x784) $x787) $x790) $x793)))
 (let (($x1393 (= invalidFree_2 invalidFree1_2)))
 (let (($x752 (= H_2 H4_2)))
 (let (($x749 (= AH_2 AH3_2)))
 (let (($x746 (= $p8_main0_2 $p8_main01_2)))
 (let (($x743 (= $p7_main0_2 $p7_main01_2)))
 (let (($x740 (= $p6_main0_2 $p6_main01_2)))
 (let (($x741 (and (and (= $M.0_2 $M.03_2) (= $i9_main0_2 $i9_main01_2)) $x740)))
 (let (($x753 (and (and (and (and $x741 $x743) $x746) $x749) $x752)))
 (let (($x691 (or (and (and (distinct $p8_main01_2 nil) true) (= $i9_main01_2 1)) (and (not (and (distinct $p8_main01_2 nil) true)) (= $i9_main01_2 0)))))
 (let (($x693 (= $p8_main01_2 $M.03_2)))
 (let (($x1371 (or $x693 invalidFree1_2)))
 (let (($x695 (= $M.03_2 nil)))
 (let (($x1372 (or $x695 invalidFree1_2)))
 (let (($x1373 (= invalidFree1_2 invalidFree_1)))
 (let (($x1374 (and invalidFree_1 $x1373)))
 (let (($x697 (= H4_2 h3_2)))
 (let (($x698 (= $p7_main01_2 l6_2)))
 (let (($x702 (= H_1 (uplus h3_2 (pt l6_2 (Pt_R_1 d5_2))))))
 (let (($x705 (= d3_2 1)))
 (let (($x707 (= $p7_main01_2 l5_2)))
 (let (($x711 (= H_1 (uplus h3_2 (pt l5_2 (Pt_R_1 d4_2))))))
 (let (($x714 (= d3_2 2)))
 (let (($x716 (and (=> $x714 (and (and $x711 $x707) $x697)) (=> $x705 (and (and $x702 $x698) $x697)))))
 (let (($x717 (= AH3_2 ah1)))
 (let (($x721 (= AH_1 (uplus ah1 (pt $p7_main01_2 (Pt_R_1 d3_2))))))
 (let (($x723 (and (and $x721 $x717) $x716)))
 (let (($x1381 (or (= $p7_main01_2 nil) (= ah2_2 (uplus AH_1 (pt $p7_main01_2 (Pt_R_1 d6_2)))))))
 (let (($x1382 (and $x1381 invalidFree1_2)))
 (let (($x725 (= $p7_main01_2 $p6_main01_2)))
 (let (($x1385 (or $x725 invalidFree_1)))
 (let (($x727 (= $p6_main01_2 $M.0_1)))
 (let (($x1386 (or $x727 invalidFree_1)))
 (let (($x1389 (and (and (and $x1386 $x1385) (or (or $x1382 (and $x723 $x1373)) $x1374)) $x1372)))
 (let (($x1395 (and (and (and $x1389 $x1371) (or $x691 invalidFree1_2)) (and $x753 $x1393))))
 (let (($x683 (= $i5_main0_1 1)))
 (let (($x684 (and $x666 $x683)))
 (let (($x1450 (and (=> $x684 (and (and (and $x1395 $x794) $x801) $x808)) (=> $x839 $x1448))))
 (let (($x1362 (= invalidFree_1 invalidFree_0)))
 (let (($x662 (= $r_main0_1 $r_main0_0)))
 (let (($x658 (= $i9_main0_1 $i9_main0_0)))
 (let (($x663 (and (and (and true (= $i5_main0_1 $i5_main0_0)) $x658) $x662)))
 (let (($x649 (= $p8_main0_1 $p8_main0_0)))
 (let (($x645 (= $p7_main0_1 $p7_main0_0)))
 (let (($x641 (= $p6_main0_1 $p6_main0_0)))
 (let (($x637 (= $p17_main0_1 $p17_main0_0)))
 (let (($x633 (= $p16_main0_1 $p16_main0_0)))
 (let (($x629 (= $p15_main0_1 $p15_main0_0)))
 (let (($x625 (= $p14_main0_1 $p14_main0_0)))
 (let (($x621 (= $p13_main0_1 $p13_main0_0)))
 (let (($x617 (= $p12_main0_1 $p12_main0_0)))
 (let (($x613 (= $p11_main0_1 $p11_main0_0)))
 (let (($x622 (and (and (and (and true (= $p10_main0_1 $p10_main0_0)) $x613) $x617) $x621)))
 (let (($x646 (and (and (and (and (and (and $x622 $x625) $x629) $x633) $x637) $x641) $x645)))
 (let (($x650 (and $x646 $x649)))
 (let (($x601 (= H_1 H3_1)))
 (let (($x598 (= AH_1 AH2_1)))
 (let (($x595 (= $p4_main0_1 $p4_main01_1)))
 (let (($x592 (= $p3_main0_1 $p3_main01_1)))
 (let (($x589 (= $p2_main0_1 $p2_main01_1)))
 (let (($x586 (= $p1_main0_1 $p1_main01_1)))
 (let (($x587 (and (and (= $M.0_1 $M.02_1) (= $p0_main0_1 $p0_main01_1)) $x586)))
 (let (($x602 (and (and (and (and (and $x587 $x589) $x592) $x595) $x598) $x601)))
 (let (($x1357 (and $x602 true)))
 (let (($x530 (and (= H2_1 (uplus h1_1 (pt $p4_main01_1 (Pt_R_0 l3_1)))) (= H3_1 (uplus h1_1 (pt $p4_main01_1 (Pt_R_0 $p2_main01_1)))))))
 (let (($x1342 (or $x530 invalidFree_0)))
 (let (($x541 (= $p4_main01_1 $M.02_1)))
 (let (($x1343 (or $x541 invalidFree_0)))
 (let (($x543 (= $p3_main01_1 $p2_main01_1)))
 (let (($x1344 (or $x543 invalidFree_0)))
 (let (($x547 (= AH2_1 (uplus AH1_1 (pt l2_1 (Pt_R_1 2))))))
 (let (($x548 (= $p2_main01_1 l2_1)))
 (let (($x552 (= H2_1 (uplus H1_1 (pt l2_1 (Pt_R_1 d2_1))))))
 (let (($x554 (and (and $x552 $x548) $x547)))
 (let (($x556 (= $M.02_1 $p0_main01_1)))
 (let (($x1346 (or $x556 invalidFree_0)))
 (let (($x558 (= $p1_main01_1 $p0_main01_1)))
 (let (($x1347 (or $x558 invalidFree_0)))
 (let (($x562 (= AH1_1 (uplus AH_0 (pt l1_1 (Pt_R_1 1))))))
 (let (($x563 (= $p0_main01_1 l1_1)))
 (let (($x567 (= H1_1 (uplus H_0 (pt l1_1 (Pt_R_1 d1_1))))))
 (let (($x569 (and (and $x567 $x563) $x562)))
 (let (($x571 (= $M.01_1 nil)))
 (let (($x1349 (or $x571 invalidFree_0)))
 (let (($x1353 (and (and (and (and $x1349 (or $x569 invalidFree_0)) $x1347) $x1346) (or $x554 invalidFree_0))))
 (let (($x1360 (and (and (and (and (and (and $x1353 $x1344) $x1343) $x1342) $x1357) $x650) $x663)))
 (let (($x494 (= loc_0 1)))
 (let (($x1340 (not invalidFree_0)))
 (let (($x496 (and (and (= H_0 emp) (= AH_0 emp)) $x494)))
 (let (($x1451 (and (and (and $x496 $x1340) (=> $x494 (and (and $x1360 $x1362) $x666))) $x1450)))
 (let (($x1531 (and (and (and $x1451 $x1499) $x1519) (=> $x1244 (and (and (and $x1524 $x1328) $x1527) $x1334)))))
 (and $x1531 invalidFree_5)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
(check-sat)
