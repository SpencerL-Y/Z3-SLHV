...
// SLHV
bool cmd_context::logic_has_slhv() const {
    return !has_logic() || smt_logics::logic_has_slhv(m_logic);
}

void cmd_context::init_manager_core(bool new_manager) {
    #ifdef SLHV_DEBUG
    std::cout << "init manager core" << std::endl;
    #endif
    SASSERT(m_manager != 0);
    ...
    if (new_manager) {
        ...
        // SLHV
        #ifdef SLHV_DEBUG
        std::cout << "slhv decl plugin registered:  " << !has_logic() << std::endl;
        #endif
        register_plugin(symbol("slhv"), alloc(slhv_decl_plugin), logic_has_slhv());
    }
    else {
        ...
        // SLHV
        load_plugin(symbol("slhv"),      logic_has_slhv(), fids);
...
void cmd_context::init_manager() {
    ...
    if (m_manager_initialized) {
        ...
        std::cout << "cmd context init external manager" << std::endl;
...
psort_decl * cmd_context::find_psort_decl(symbol const & s) const {
            ...
            #ifdef SLHV_DEBUG
            std::cout << "find_psort_decl: " << s << std::endl;
            #endif 
    psort_decl * p = nullptr;
    m_psort_decls.find(s, p);
    #ifdef SLHV_DEBUG
            std::cout << "find_psort_decl end " << std::endl;
            #endif 
    return p;
}
...
void cmd_context::reset(bool finalize) {    
    ...
    if (m_manager) {
        ...
        if (m_own_manager) {
            ...
            if (!finalize) {
            #ifdef SLHV_DEBUG
            std::cout << "init external manager at cmd context eset" << std::endl;
            #endif
                init_external_manager();
            }
            else
...
void cmd_context::push() {
    m_check_sat_result = nullptr;
    #ifdef SLHV_DEBUG
    std::cout << "init manager at push" << std::endl;
    #endif
    init_manager();
...
void cmd_context::check_sat(unsigned num_assumptions, expr * const * assumptions) {
    ...
    init_manager();
#ifdef SLHV_DEBUG
    std::cout << "init manager at check sat" << std::endl;
    std::cout << "before_check_sat:"; dump_assertions(std::cout); std::cout << std::endl;
#endif
    ...
    if (m_opt && !m_opt->empty()) {
        ...
        if (!get_opt()->is_pareto() || is_clear) {
            ...
            #ifdef SLHV_DEBUG
                    std::cout << "m_assertion_name: " <<  m_assertion_names[i] << "m_assertion: " << m_assertions[i] << std::endl;
            #endif
            ...
            #ifdef SLHV_DEBUG
                    std::cout << "m_assertion: " << m_assertions[i] << std::endl;
            #endif
