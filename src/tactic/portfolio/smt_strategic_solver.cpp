...
#include "tactic/smtlogics/slhv_tactic.h"
...
tactic * mk_tactic_for_logic(ast_manager & m, params_ref const & p, symbol const & logic) {
    if (logic=="QF_UF")
        ...
        return mk_slhv_tactic(m, p);
    else {
        std::cout << "tactic: mk default tactic" << std::endl;
...
public:
    ...
    solver * operator()(ast_manager & m, params_ref const & p, bool proofs_enabled, bool models_enabled, bool unsat_core_enabled, symbol const & logic) override {
        ...
        if (!t) {
            #ifdef SLHV_DEBUG
            std::cout << "mk special solver for logic " << std::endl;
            #endif
            solver* s = mk_special_solver_for_logic(m, p, l);

            #ifdef SLHV_DEBUG
            std::cout << "mk special solver for logic end " << std::endl;
            #endif
            if (s) return s;
        }
        if (!t) {
            #ifdef SLHV_DEBUG
            std::cout << "mk tactic  for logic " << std::endl;
            #endif
            t = mk_tactic_for_logic(m, p, l);
            #ifdef SLHV_DEBUG
            std::cout << "mk tactic  for logic end" << std::endl;
            #endif
        }
        #ifdef SLHV_DEBUG
            std::cout << "mk combined solver " << std::endl;
        #endif
