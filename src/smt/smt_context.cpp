...
#define SMALL_NUM_PARENTS 3
    ...
    */
    ...
    bool context::propagate() {
        ...
        #ifdef SLHV_DEBUG
        std::cout  << "propagating... " << m_qhead << ":" << m_assigned_literals.size() << "\n";
        #endif
        while (true) {
            if (inconsistent())
                return false;
...
//                scoped_suspend_rlimit _suspend_cancel(m.limit(), at_base_level());
    ...
    */
    bool context::decide() {
        ...
        if (at_search_level() && !m_tmp_clauses.empty()) {
            #ifdef SLHV_DEBUG
            std::cout << "decide: at_search_level and m_tmp_clauses is not empty" << std::endl;
            #endif
        ...
        #ifdef SLHV_DEBUG
        std::cout << "decide: does not have_split_candidate" << std::endl;
        #endif
            lbool phase = l_undef;
        ...
        #ifdef SLHV_DEBUG
        std::cout << "splitting, lvl: " << m_scope_lvl << "\n";
        #endif
    ...
    */
    ...
    lbool context::decide_clause() {
        #ifdef SLHV_DEBUG
        std::cout << "decide_clause()" << std::endl;
        #endif 
        if (m_tmp_clauses.empty()) return l_true;
        for (auto & tmp_clause : m_tmp_clauses) {
            #ifdef SLHV_DEBUG
            std::cout << "decide clause: ";
            tmp_clause.first->display(std::cout, m, m_bool_var2expr.data());
            #endif
    ...
    */
    ...
    void context::setup_components() {
        ...
        for (theory* th : m_theory_set) {
            #ifdef SLHV_DEBUG
            std::cout << "theory setup(): " << th->get_name() << std::endl;
            #endif
            th->setup();
        }
    }
    ...
    lbool context::search() {
        ...
        #ifdef SLHV_DEBUG
        std::cout << "after_init_search: " << std::endl;
        std::cout << "----------------------------------" << std::endl;
        display(std::cout);
        std::cout << "----------------------------------" << std::endl;
        #endif 
    ...
    lbool context::bounded_search() {
        #ifdef SLHV_DEBUG
        std::cout << "bounded_search()" << std::endl;
        #endif 
        unsigned counter = 0;
