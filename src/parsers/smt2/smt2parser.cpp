...
namespace smt2 {
    ...
    class parser {
        ...
        //SLHV
        symbol               m_declare_hvar;
        symbol               m_declare_locvar;
        ...
        void parse_sort(char const* context) {
            #ifdef SLHV_DEBUG
            std::cout << "parse sort, curr: " << m_curr << std::endl;
            #endif 
            ...
            #ifdef SLHV_DEBUG
            std::cout << "parse sort end, curr: " << m_curr << std::endl;
            #endif 
        }
        ...
        void parse_declare_datatypes() {
            #ifdef SLHV_DEBUG
            std::cout << "parse declare datatypes" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_declare_datatype() {
            #ifdef SLHV_DEBUG
            std::cout << "parse declare datatype" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void push_expr_frame(expr_frame * curr) {
            ...
            if (curr_is_identifier()) {
                ...
                #ifdef SLHV_DEBUG
                std::cout << "push_expr_frame(), curr_id(): " << curr_id() << "\n";
                #endif
                if (curr_id_is_let()) {
                    push_let_frame();
                }
        ...
        void pop_expr_frame() {
            ...
            next(); // consume ')'
            #ifdef SLHV_DEBUG
            std::cout << "pop expr frame: " << fr->m_kind << std::endl;
            #endif
        }
        ...
        void parse_declare_sort() {
            #ifdef SLHV_DEBUG
            std::cout << "parse declare sort" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_define_sort() {
            #ifdef SLHV_DEBUG
            std::cout << "parse define sort" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_define_fun() {
            #ifdef SLHV_DEBUG
            std::cout << "parse define fun" << std::endl;
            #endif
            parse_define(true);
        }
        ...
        void parse_model_add() {
            #ifdef SLHV_DEBUG
            std::cout << "parse model add" << std::endl;
            #endif
            parse_define(false);
        }
        ...
        void parse_model_del() {
            #ifdef SLHV_DEBUG
            std::cout << "parse model del" << std::endl;
            #endif
            next();
            symbol id = curr_id();
        ...
        void parse_define_fun_rec() {
            ...
            #ifdef SLHV_DEBUG
            std::cout << "parse declare fun rec" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_define_funs_rec() {
            ...
            #ifdef SLHV_DEBUG
            std::cout << "parse declare funs rec" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_define_const() {
            #ifdef SLHV_DEBUG
            std::cout << "parse define const" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_declare_fun() {
            #ifdef SLHV_DEBUG
            std::cout << "parse declare fun" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_declare_const() {
            #ifdef SLHV_DEBUG
            std::cout << "parse declare const" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        bool check_hvar_sort(sort* hvar_sort) {
            if(hvar_sort->get_name() == "IntHeap"){
                return true;
            } else {
                return false;
            }
        }
        void parse_declare_hvar() {
            #ifdef SLHV_DEBUG
            std::cout << "parse declare hvar" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
            SASSERT(curr_id() == m_declare_hvar);
            next();
            check_nonreserved_identifier("invalid hvar declaration, symbol expected");
            symbol id = curr_id();
            #ifdef SLHV_DEBUG
            ...
            next();
            #ifdef SLHV_DEBUG
            std::cout << "parse declare hvar end, curr: " << m_curr << std::endl;
            #endif
        }

        bool check_locvar_sort(sort* locvar_sort) {
            if(locvar_sort->get_name() == "IntLoc") {
                return true;
            } else {
                return false;
            }
        ...
        void parse_declare_locvar() {
            #ifdef SLHV_DEBUG
            std::cout << "parse delcare locvar" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
            SASSERT(curr_id() == m_declare_locvar);
            next();
            check_nonreserved_identifier("invalid locvar declaration, symbol expected");
            symbol id = curr_id();
            #ifdef SLHV_DEBUG
            ...
            next();
            #ifdef SLHV_DEBUG
            std::cout << "parse declare locvar end, curr: " << m_curr << std::endl;
            #endif
        }
    ...
    std::cout << "parse_assert" << std::endl;
        ...
        void parse_check_sat() {
            #ifdef SLHV_DEBUG
                std::cout << "parse_check_sat" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_check_sat_assuming() {
            #ifdef SLHV_DEBUG
            std::cout << "parse check sat assuming" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_get_value() {
            #ifdef SLHV_DEBUG
            std::cout << "parse get value" << std::endl;
            #endif
            SASSERT(curr_is_identifier());
        ...
        void parse_next_cmd_arg() {
            ...
            #ifdef SLHV_DEBUG
            std::cout << "parse next cmd arg kind: " << m_curr_cmd->next_arg_kind(m_ctx) << std::endl;
            #endif
            switch (k) {
            case CPK_UINT: {
        ...
        void parse_ext_cmd(int line, int pos) {
            symbol s = curr_id();
            #ifdef SLHV_DEBUG
            std::cout << "parse ext cmd: " << s << std::endl;
            #endif
        ...
        void parse_cmd() {
            ...
            // next() will call scan_core() and the next token will be fed to m_curr
            next();
            ...
            if (s == m_declare_locvar) {
                parse_declare_locvar();
                return;
            }
            if (s == m_check_sat) {
                parse_check_sat();
    ...
    public:
        parser(cmd_context & ctx, std::istream & is, bool interactive, params_ref const & p, char const * filename=nullptr):
            ...
            // SLHV
            m_declare_hvar("declare-hvar"),
            m_declare_locvar("declare-locvar"),
        ...
        bool operator()() {
            ...
            try {
                scan_core();
                // scan_core will each time scan a token to m_curr
            }
            catch (scanner_exception & ex) {
                error(ex.msg());
                if (!sync_after_error())
            ...
            while (true) {
                try {
                    ...
                    while (true) {
                        switch (curr()/*m_curr*/) {
                        case scanner::LEFT_PAREN:
                        // parse_cmd() will parse the statements into expressions according to the token read, these expressions will later be stored into m_ctx
                            parse_cmd();
                            break;
                        case scanner::EOF_TOKEN:
